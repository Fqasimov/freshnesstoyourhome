 ...
 *
 * AZ is the source of truth and the default; RU and EN are translations of it.
 * Every key must exist in all three — a missing one falls back to AZ rather
 * than showing a blank, because a customer seeing nothing is worse than a
...
export async function setLang (next) {
  ...
  try { await Preferences.set({ key: 'lang', value: next }) } catch { /* not fatal */ }
}

/** Pick the current language out of a {az, ru, en} map from the API. */
export function pick (map, fallback = '') {
  if (!map) return fallback
  return map[lang.value] ?? map.az ?? fallback
}
...
export function t (key, vars) {
  const table = MESSAGES[lang.value] ?? MESSAGES.az
  let out = table[key] ?? MESSAGES.az[key] ?? key
...
/** Money arrives from the API in qəpik and is displayed here, never computed. */
export function money (minor, currency = 'AZN') {
  if (minor == null) return ''
  const sign = minor < 0 ? '-' : ''
  const abs = Math.abs(minor)
  const major = Math.floor(abs / 100).toLocaleString(
    lang.value === 'ru' ? 'ru-RU' : lang.value === 'az' ? 'az-AZ' : 'en-GB'
  )
  return `${sign}${major}.${String(abs % 100).padStart(2, '0')} ${currency}`
}
